...
input_key_kitty(struct screen *s, struct bufferevent *bev,key_code key)
{
	...
	if (KEYC_IS_USER(onlykey) || KEYC_IS_SPECIAL(onlykey) ||
	    KEYC_IS_MOUSE(onlykey)) {
		return (-1);
	}
...
emit_escapes:
    ...
    if (final == 'u' || final == '~') {
		...
		/* CSI number ; modifiers u */
		if(modifier==1){
			xsnprintf(tmp, sizeof tmp, "\033[%llu%c",number,final);
		}else{
			xsnprintf(tmp, sizeof tmp, "\033[%llu;%u%c",number,modifier,final);
		}
